   ...
   begin
      ...
      case Source is
         ...
         when SYSCLK_SRC_HSI =>
            ...
            Result.I2CCLK := Param.HSI_Clock;

         --  HSE as source
         when SYSCLK_SRC_HSE =>
            Result.SYSCLK := Param.HSE_Clock;
            Result.I2CCLK := Param.HSE_Clock;
         ...
         when SYSCLK_SRC_PLL =>
            declare
               ...
               PLLSRC : constant PLL_Source :=
                 PLL_Source'Val (RCC_Periph.CFGR.PLLSRC);
            begin
               --  PLL Source Mux
               case PLLSRC is
                  when PLL_SRC_HSE =>
                     --  HSE/PREDIV selected as PLL input clock
                     PLLCLK := Param.HSE_Clock / Plld * Pllm;
                  when PLL_SRC_HSI =>
                     --  HSI/2 selected as PLL input clock
                     PLLCLK := Param.HSI_Clock / 2 * Pllm;
               end case;
      ...
      declare
         Activate_Pll : constant Bit := RCC_Periph.CR.PLLON;
         TIM1_Pll     : constant Bit := RCC_Periph.CFGR3.TIM1SW;
         HRTIM1_Pll   : constant Bit := RCC_Periph.CFGR3.HRTIM1SW;
      begin
         --  TIM1 source Mux
         if (Activate_Pll = 1) and (TIM1_Pll = 1) then
         ...
         --  HRTIM1 source Mux
         if (Activate_Pll = 1) and (HRTIM1_Pll = 1) then
